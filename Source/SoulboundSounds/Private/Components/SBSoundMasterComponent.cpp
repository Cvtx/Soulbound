...
USBSoundMasterComponent::USBSoundMasterComponent()
{
	PrimaryComponentTick.bCanEverTick = true;	
}

void USBSoundMasterComponent::BeginPlay()
{
	Super::BeginPlay();
...
void USBSoundMasterComponent::TickComponent(const float DeltaTime, const ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void USBSoundMasterComponent::RecastOwner()
{
	if (!GetOwner())
	{
		return;
...
void USBSoundMasterComponent::TestAllFeetForCollision()
{
	...
	GetWorld()->GetTimerManager().SetTimer(
		...
		this,
		&USBSoundMasterComponent::OpenAutoFootStepGate,
		MinTimeCycle,
		false);
}

void USBSoundMasterComponent::OpenAutoFootStepGate()
{
	bAutoFootStepGateOpen = true;
}

void USBSoundMasterComponent::CloseAutoFootStepGate()
{
	bAutoFootStepGateOpen = false;
}

bool USBSoundMasterComponent::TryTraceFootstep(const FName Key)
{
...
void USBSoundMasterComponent::FootstepEffect(const FHitResult& Hit,UAudioComponent*& Sound, UNiagaraSystem*& Particle)
{
...
void USBSoundMasterComponent::FootstepSoundEffect(USoundCue* SoundToPlay, const FVector& Location ,UAudioComponent*& FootStepSound) const
{
...
void USBSoundMasterComponent::FootstepParticleEffect(UNiagaraSystem* ParticleToSpawn, const FVector& Location, const FVector& Normal, UNiagaraSystem*& NiagaraSystem) const
{
...
float USBSoundMasterComponent::OverwriteCalcVolume_Implementation() const
{
	if (!IsValid(FootstepSetup))
	{
		#if WITH_EDITOR
...
FVector USBSoundMasterComponent::OverwriteParticleScale_Implementation() const
{
	return GetOwner()->GetActorScale3D();
}

void USBSoundMasterComponent::OverwritePickSurface_Implementation(const TEnumAsByte<EPhysicalSurface>& InSurface, TEnumAsByte<EPhysicalSurface>& OutSurface)
{
